...
class PreviewManagerMixin:
    ...
    def update_face_features_display(self):
        ...
        try:
            ...
            # 고급 모드이고 변형된 랜드마크가 있으면 변형된 랜드마크 사용
            detected = False
            
            # 고급 모드: 변형된 랜드마크 사용 (가이드축 방향 확대 표시)
            if (hasattr(self, 'use_landmark_warping') and 
                self.use_landmark_warping.get() and 
                hasattr(self, '_last_transformed_custom_landmarks') and 
                self._last_transformed_custom_landmarks is not None):
                landmarks = self._last_transformed_custom_landmarks
                detected = True
                print(f"[DEBUG] update_face_features_display (원본): 변형된 랜드마크 사용")
            # 일반 모드: 원본 랜드마크 사용
            elif self.custom_landmarks is not None:
            ...
            # 편집된 이미지에 변형된 랜드마크 폴리곤 표시
            if (hasattr(self, 'use_landmark_warping') and 
                self.use_landmark_warping.get() and 
                hasattr(self, '_last_transformed_custom_landmarks') and 
                self._last_transformed_custom_landmarks is not None and
                show_polygons):
                # 백업된 변형된 랜드마크 사용
                transformed_landmarks_for_polygon = self._last_transformed_custom_landmarks
                
                # 디버그: 변형된 랜드마크 샘플 출력
                if len(transformed_landmarks_for_polygon) > 0:
                    sample_idx = 384  # left_eye 영역 인덱스
                    if sample_idx < len(transformed_landmarks_for_polygon):
                        transformed_point = transformed_landmarks_for_polygon[sample_idx]
                        print(f"[DEBUG] 편집된 폴리곤 랜드마크 샘플: idx={sample_idx}, point=({transformed_point[0]:.2f},{transformed_point[1]:.2f})")
                
                # 편집된 캔버스에 변형된 랜드마크 폴리곤 그리기
                self._draw_landmark_polygons(
                    self.canvas_edited,
                    self.edited_image if self.edited_image else self.current_image,
                    transformed_landmarks_for_polygon,
                    self.canvas_edited_pos_x,
                    self.canvas_edited_pos_y,
                ...
                print(f"[DEBUG] 편집된 캔버스에 변형된 폴리곤 그리기 완료")
