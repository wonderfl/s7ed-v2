...
class DrawingMixin(AllTabDrawerMixin):
    """폴리곤 그리기 기능 Mixin"""
    ...
    def __init__(self):
        ...
        # v2에서는 단순화된 구조 사용 - iris_renderer 제거됨
    ...
    def _draw_landmark_polygons(self, canvas, image, face_landmarks, pos_x, pos_y, items_list, color, current_tab, iris_landmarks=None, iris_centers=None, force_use_custom=False, highlight_indices=None):
        ...
        try:
            ...
            # 디버그: 전달된 landmarks 확인
            if len(landmarks) > 384:
                sample_point = landmarks[384]
                print(f"[폴리곤렌더러] _draw_landmark_polygons 전달된 좌표: idx=384, point=({sample_point[0]:.2f},{sample_point[1]:.2f})")
            ...
            # 전달된 landmarks를 직접 사용 (변형된 좌표 보존)
            ...
            if iris_centers is not None and len(iris_centers) == 2:
                ...
                # 전달된 landmarks + iris_centers 사용
                landmarks_with_centers = list(landmarks)
                landmarks_with_centers.extend(iris_centers)  # +2개 = 470개
                landmarks = landmarks_with_centers
