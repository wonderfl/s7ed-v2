...
from ..region import _get_eye_region, _get_mouth_region, _get_nose_region, _get_region_pivot, _get_region_bbox

from utils.logger import log, debug, info, warning, error
from gui.FaceForge.utils.debugs import DEBUG_APPLY_SLIDERS, DEBUG_APPLY_TRANSFORM, DEBUG_REGION_ADJUST
...
def _compute_region_bbox_and_center(region_name, landmarks, img_width, img_height, pivot_x=0.0, pivot_y=0.0):
...
def adjust_region_size(image, region_name, size_x=1.0, size_y=1.0, pivot_x=0.0, pivot_y=0.0, landmarks=None, blend_ratio=1.0):
    """
    ...
    Args:
        ...
        pivot_x: 중심점 오프셋 X (픽셀, 기본값: 0.0)
        pivot_y: 중심점 오프셋 Y (픽셀, 기본값: 0.0)
    ...
    # 디버그 출력
    if DEBUG_REGION_ADJUST:
        debug("adjust_region_size", 
            f"region_name={region_name}, "
            f"size({size_x},{size_y}), "
            f"pivot({pivot_x},{pivot_y})"
        )
...
def adjust_region_size_with_axis(
    ...
    if DEBUG_REGION_ADJUST:
        debug( "adjust_region_size_with_axis", 
            f"region={region_name}, size({size_x:.3f},{size_y:.3f}), "
            f"pivot({pivot_x},{pivot_y}), angle={math.degrees(guide_angle):.1f}°"
        )
    ...
    if guide_angle is None or (abs(size_x - 1.0) < 0.01 and abs(size_y - 1.0) < 0.01):
        return adjust_region_size(
            ...
            pivot_x, pivot_y, landmarks, blend_ratio
        )
    ...
    try:
        ...
        if DEBUG_REGION_ADJUST:
            debug( "adjust_region_size_with_axis",
                f"prepare region={region_name}, bbox=({x1},{y1},{x2},{y2}), angle={math.degrees(guide_angle):.1f}°, size=({size_x:.3f},{size_y:.3f})"
            )
        ...
        if DEBUG_REGION_ADJUST:
            debug("adjust_region_size_with_axis", f" cv2 applied: region={region_name}")
...
def adjust_region_position(image, region_name, position_x=0.0, position_y=0.0, 
                          pivot_x=0.0, pivot_y=0.0, landmarks=None):
    """
    ...
    if DEBUG_REGION_ADJUST:
        debug("adjust_region_position", f"region={region_name}")
    ...
    try:
        ...
        # 부위 중심점 계산 (오프셋 포함)
        center = _get_region_pivot(region_name, landmarks, pivot_x, pivot_y)
        if center is None:
            return image
        ...
        if region_name in ['left_eye', 'right_eye']:
            ...
            if key_landmarks is None:
                return image
            eye_name = 'left' if region_name == 'left_eye' else 'right'
            eye_region, _ = _get_eye_region(key_landmarks, img_width, img_height, eye_name, landmarks, 0.3, pivot_x, pivot_y)
            x1, y1, x2, y2 = eye_region
            ...
            if key_landmarks is None:
                return image
            nose_region, _ = _get_nose_region(key_landmarks, img_width, img_height, landmarks, 0.3, pivot_x, pivot_y)
            x1, y1, x2, y2 = nose_region
            region_size = max(x2 - x1, y2 - y1) / 2
        else:
