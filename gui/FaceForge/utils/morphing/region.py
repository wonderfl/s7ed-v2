...
from gui.FaceForge.utils.debugs import DEBUG_REGION_PIVOTS
...
def _get_region_pivot(region_name, landmarks, pivot_x=0.0, pivot_y=0.0):
    """
    부위별 중심점 계산 (오프셋 포함)
    ...
    if DEBUG_REGION_PIVOTS:
        print("[_get_region_center]", f"region_name: {region_name}, landmarks: {len(landmarks)} available: {_landmarks_available}")
...
def _get_region_bbox(region_name, landmarks, img_width, img_height, padding_ratio=0.1, pivot_x=0.0, pivot_y=0.0):
    """
