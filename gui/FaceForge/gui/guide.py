...
from gui.FaceForge.utils.debugs import DEBUG_GUIDE_LINES
...
class GuideLinesManager:
    ...
    def __init__(self, parent):
        ...
        self.guide_line_settings = {
            ...
            'color': '#A52A2A',         # 지시선 색상 (부드러운 빨간색)
            'width': 1,                 # 선 너비
            'dash': (2, 1),             # 점선 패턴 (원래대로)
            'max_line_ratio': 0.25,     # 캔버스 대비 최대 길이 비율
            'fixed_line_length': None,  # 고정 길이 (None이면 비율 사용)            
        }

        self._force_guide_scaling = False  # 강제 지시선 스케일링 플래그

        if DEBUG_GUIDE_LINES:
            print("[GuideLinesManager]"," : initialized..")        
        self._ensure_state_initialized()

    def calculate_eye_center_line(self, landmarks, img_width, img_height, 
                                scale_x, scale_y, pos_x, pos_y, bbox_width) -> Optional[Tuple[float, float, float, float]]:
        ...
        try:
            ...
            if bbox_width:
                line_length = bbox_width * scale_x
            else:
                if self.guide_line_settings['fixed_line_length']:
                    line_length = self.guide_line_settings['fixed_line_length']
                else:
                    max_canvas_size = max(canvas_width, canvas_height)
                    line_length = max_canvas_size * scale_x * self.guide_line_settings['max_line_ratio']
    ...
    def calculate_perpendicular_lines(self, eye_center_info, img_width, img_height,
                                   scale_x, scale_y, pos_x, pos_y, bbox_width=None) -> List[Tuple[float, float, float, float]]:
        ...
        try:
            ...
            # line_length = face_size * 0.08
            if bbox_width:
                line_length = bbox_width * scale_x * 0.20
    ...
    def calculate_nose_center_line(self, landmarks, img_width, img_height, 
                                 scale_x, scale_y, pos_x, pos_y, eye_center_info=None,
                                 bbox_width=None ) -> Optional[Tuple[float, float, float, float]]:
        ...
        try:
            ...
            line_x1 = canvas_lip_x - dir_x * line_length * 3 / 4
            line_y1 = canvas_lip_y - dir_y * line_length * 3 / 4
            line_x2 = canvas_lip_x + dir_x * line_length * 1 / 4
            line_y2 = canvas_lip_y + dir_y * line_length * 1 / 4
    ...
    def calculate_lip_center_line(self, landmarks, img_width, img_height, 
                                 scale_x, scale_y, pos_x, pos_y, eye_center_info=None, bbox_width=None ) -> Optional[Tuple[float, float, float, float]]:
        ...
        try:
            ...
            # 입술 중심선의 시작점과 끝점
            line_x1 = canvas_lip_x - dir_x * line_length / 3
            line_y1 = canvas_lip_y - dir_y * line_length / 3
            line_x2 = canvas_lip_x + dir_x * line_length / 3
            line_y2 = canvas_lip_y + dir_y * line_length / 3
    ...
    def draw_guide_lines(self, canvas, landmarks, img_width, img_height,
        ...
        if DEBUG_GUIDE_LINES:
            info("draw_guide_lines", f"image({img_width}x{img_height}), pos({pos_x},{pos_y}), canvas_type: {canvas_type}")
        ...
        try:
            ...
            # bbox 정보 가져오기
            bbox = None
            has_landmark_manager = hasattr(self.parent, 'landmark_manager')
            if has_landmark_manager:
                bbox = self.parent.landmark_manager.get_original_bbox(img_width, img_height)
            
            bbox_width = 100
            if bbox:
                 min_x, _, max_x, _ = bbox
                 bbox_width = (max_x - min_x) * 0.5

            if DEBUG_GUIDE_LINES:
                print("[draw_guide_lines]", f"bbox: {bbox}, {has_landmark_manager}, bbox_width: {bbox_width}")

            eye_center_info = self.calculate_eye_center_line(
                landmarks, img_width, img_height, scale_x, scale_y, pos_x, pos_y, bbox_width
            )
            ...
            if self.guide_line_settings['vertical_line']:
                perpendicular_lines = self.calculate_perpendicular_lines(
                    eye_center_info, img_width, img_height, scale_x, scale_y, pos_x, pos_y, bbox_width
                )
            ...
            if self.guide_line_settings['nose_center_line']:
                nose_line_info = self.calculate_nose_center_line(
                    landmarks, img_width, img_height, scale_x, scale_y, pos_x, pos_y, eye_center_info, bbox_width
                )
