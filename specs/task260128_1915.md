...
- 현재 `apply_editing`은 여전히 모놀리식 흐름을 유지하고 있으며, 체크리스트 1~2단계(탭 생성기/런처 정리, 상태 시그니처 정의)부터 순차적으로 진행해야 한다. 다음 작업은 `_prepare_editing_context`/`_ensure_landmark_sources` 추출과 상태 시그니처 설계다.
