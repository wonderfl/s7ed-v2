...
- [x] 함수 시그니처(예: `update_face_edit_display(image=False, landmarks=False, overlays=False, guide_lines=False)`) 정의
- [x] 내부에서 필요한 기존 함수 호출 정리(`show_edited_preview`, `update_face_features_display`, eye/lip/bbox 오버레이, guide lines 등)
- [x] 모든 이벤트가 이 함수를 통해서만 화면을 갱신하도록 기존 직접 호출을 정리 *(줌·캔버스·편집 파이프라인 + 파일 로드(load_image) + 폴리곤 드래그 경로 전환 완료, 여전히 슬라이더/옵션/정렬/기타 드래그 후속 경로는 남음)*
- [x] `PreviewManagerMixin`( `gui/face_edit/preview.py` ) 내에서 디스플레이 전용 책임으로 구현하고, 이미지/랜드마크/오버레이 각각의 최소 갱신 플래그를 인자로 받아 조건부로 실행되도록 설계
- [x] 이벤트 소스 플래그와 연계하여 슬라이더/옵션/드래그별 필요한 갱신 범위를 명시하고, `_perform_morphing_update`에서는 이 함수를 호출만 하도록 단순화 *(플래그 설계는 완료, 실제 호출부 연동은 단계 4와 함께 진행 예정)*

#### `update_face_edit_display` 설계 메모
- **시그니처/플래그**: `update_face_edit_display(self, image=True, landmarks=False, overlays=False, guide_lines=False, *, force_original=False)`
  - `image=True`일 때만 `show_edited_preview` 호출 (원본 미리보기는 입력 이미지가 바뀐 경우에만 손대도록 guard)
  - `landmarks=True`일 때 `update_face_features_display` 호출, 그렇지 않으면 skip
  - `overlays=True`일 때 eye/lip/bbox/폴리곤 overlay를 갱신함. 폴리곤은 `show_landmark_polygons` 플래그를 보고 `_draw_landmark_polygons`만 호출.
  - `guide_lines=True`이면 `guide_lines_manager.draw_guide_lines`만 갱신. 확대/이동 이벤트에서는 이 플래그만 켜서 고비용 렌더를 피함.
  - `force_original=True`일 때만 `show_original_preview` 실행하여 불필요한 원본 캔버스 리렌더를 피함.
- **내부 플로우**:
  1. 이미지 위치 저장(원본/편집) → 필요 시 `show_original_preview`, `show_edited_preview` → `self._last_preview_update_signature` 캐시
  2. `landmarks` 플래그 체크 후 `update_face_features_display`
  3. `overlays` 플래그 체크 후 eye/lip/bbox/폴리곤 순으로 최소 호출
  4. `guide_lines` 플래그 체크 후 `guide_lines_manager.draw_guide_lines`
  5. 마지막에 `_last_change_source = 'none'`으로 리셋
- **연계 규칙**:
  - `Canvas`/줌 이벤트: `update_face_edit_display(image=False, landmarks=True, overlays=False, guide_lines=True, force_original=False)`처럼 일부 플래그만 켠다.
  - 슬라이더/드래그: `image=True, landmarks=True, overlays=True, guide_lines=True`
  - 옵션 토글: `image=False, landmarks=True, overlays=True` (시그니처 비교 결과에 따라 필요 시 `image=True`)

### 단계 4: 고급 모드 이미지 재계산 보장
- [x] 슬라이더 이벤트 시 `apply_polygon_drag_final(force_slider_mode=True)` 이후에도 이미지 갱신 경로가 실행되도록 리팩터링
- [x] 옵션 변경 시에는 `force_slider_mode=False` + 부분 갱신만 수행하도록 분기
- [x] 중복 폴리곤 렌더 제거(필요 시 `_apply_common_sliders_to_landmarks` 내 렌더 호출 조건부 처리)
