...
**작업 완료 일시**: 2026-01-19 (코드 레벨 완료, 사용자 테스트 대기)
...
1. **하위 호환성 유지**: 기존 코드가 동작하도록 property 사용
2. **점진적 전환**: 한 번에 모든 코드를 변경하지 않고 단계적으로
3. **테스트 강화**: 각 단계마다 충분한 테스트 필요
4. **직접 인덱스 접근 우선 수정**: property 적용 전에 직접 인덱스 접근 코드 수정 필수
5. **안전한 롤백**: 각 단계별 커밋으로 문제 발생 시 쉽게 되돌리기
