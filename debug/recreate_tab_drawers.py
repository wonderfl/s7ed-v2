"""
tab_drawers.py 재생성 스크립트
원본 파일에서 직접 읽어서 정확히 추출
"""
import re

def recreate_tab_drawers():
    """tab_drawers.py 파일 재생성"""
    ...
    # 각 탭의 범위 정의 (1-based line number, 원본 파일 기준)
    tab_ranges = {
        'all': (329, 1369),
        'eye': (1370, 1618),
        'nose': (1619, 1726),
    ...
    for tab_name, (start, end) in tab_ranges.items():
        ...
        for line in tab_lines:
            ...
            # 들여쓰기 조정
            # 원본은 12칸 들여쓰기 (if 문 내부)
            # 메서드 내부는 8칸 기본
            indent = len(stripped) - len(stripped.lstrip())
            content = stripped.lstrip()
            
            if indent >= 12:
                # 12칸 -> 8칸, 16칸 -> 12칸, 20칸 -> 16칸 등
    ...
    print(f"tab_drawers.py 재생성 완료: {output_file}")

if __name__ == '__main__':
    import os
    recreate_tab_drawers()
