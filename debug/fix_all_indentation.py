"""
tab_drawers.py 전체 들여쓰기 수정 스크립트
...
def fix_all_indentation():
    ...
    for tab_name, (start, end) in tab_ranges.items():
        ...
        for line in tab_lines:
            ...
            # - 16칸: try:, 주석 등 -> 메서드 내부 12칸
            # - 20칸: import, 변수 할당 등 -> 메서드 내부 16칸
            # - 24칸: if 문 등 -> 메서드 내부 20칸
            # - 28칸: 내부 코드 -> 메서드 내부 24칸
            ...
            if indent >= 16:
                # 16칸 이상: 4칸씩 줄임 (16->12, 20->16, 24->20, 28->24)
                new_indent = indent - 4
                # 주석이면 8칸으로 조정
                if is_comment and new_indent > 8:
                    new_indent = 8
                output_lines.append(' ' * new_indent + content + '\n')
            elif indent == 12:
                # 12칸: if current_tab == (이미 제거됨) 또는 주석
                # 주석은 8칸으로
                if is_comment:
    ...
    print(f"tab_drawers.py 전체 들여쓰기 수정 완료: {output_file}")
